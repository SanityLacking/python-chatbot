...
def train():
      ...
      bot.train([
        ...
        #6
        'without IELTS',#without ielts, low score, not enough score
        ...
        #7
        'recommend prestigious australian university', #recommend universities, high level universities, good universities, recommend pretigious universities
        ...
        '호주에서는 워킹홀리데이 비자를 가지고 최대 17주까지 호주에서 합법적으로 공부할 수 있습니다.',
        #19
        'how much payment part time work',
...
def search(entered_input):
      ...
      if (haystack.find('how')>=0 and haystack.find('iae eduhouse')>=0) or (haystack.find('address')>=0 and haystack.find('iae eduhouse')>=0) or (haystack.find('location')>=0 and haystack.find('iae eduhouse')>=0) or (haystack.find('where')>=0 and haystack.find('iae eduhouse')>=0):
            haystack='how can I go to iae eduhouse'
      elif (haystack.find('information')>=0 and haystack.find('application')>=0) or (haystack.find('information')>=0 and haystack.find('studying')>=0) or haystack.find('procedure')>=0 or haystack.find('process')>=0:
            haystack='information about study abroad'
      elif (haystack.find('deadline')>=0 and haystack.find('application')>=0) or (haystack.find('submit')>=0 and haystack.find('until')>=0):
            haystack='when is the application deadline'
      elif (haystack.find('how')>=0 and haystack.find('score')>=0) or (haystack.find('how')>=0 and haystack.find('ielts')>=0) or (haystack.find('how')>=0 and haystack.find('toefl')>=0):
            haystack='How much english score do I need'
      elif (haystack.find('toefl')>=0 and haystack.find('entrance')>=0) or (haystack.find('ielts')>=0 and haystack.find('entrance')>=0) or (haystack.find('english')>=0 and haystack.find('entrance')>=0):
            haystack='toefl for admission'
      elif (haystack.find('without')>=0 or haystack.find('no')>=0) and (haystack.find('ielts')>=0 or haystack.find('toefl')>=0 or haystack.find('english')>=0):
            haystack='without IELTS'
      elif (haystack.find('good universit')>=0 or haystack.find('prestigious universit')>=0  or haystack.find('high level universi')>=0):
            haystack='recommend prestigious australian university'
      elif (haystack.find('good college')>=0 or haystack.find('prestigious college')>=0  or haystack.find('high level college')>=0):
            haystack='recommend prestigious australian university'
      elif haystack.find('high school')>=0 and haystack.find('korea')>=0 and haystack.find('universit')>=0 and haystack.find('australia')>=0:
            haystack='finish high school in korea and enter australia university'
      elif haystack.find('australia')>=0 and (haystack.find('elementary school')>=0 or haystack.find('junior school')>=0 or haystack.find('high school')>=0):
            haystack='international student to enter australian school'
      elif haystack.find('after')>=0 and (haystack.find('other countr')>=0 or haystack.find('america')>=0 or haystack.find('united states')>=0 or haystack.find('canada')>=0):
            haystack='after australia high school university other country'
      elif haystack.find('university')>=0 and (haystack.find('tuition')>=0 or haystack.find('fee')>=0):
            haystack='universit cost'
      elif haystack.find('living')>=0 and (haystack.find('year')>=0 or haystack.find('cost')>=0 or haystack.find('annual')>=0 or haystack.find('expen')>=0):
      ...
      elif haystack.find('age')>=0 and haystack.find('working holiday')>=0:
      ...
      elif (haystack.find('part-time')>=0 or haystack.find('part time')>=0) and (haystack.find('how much')>=0 or haystack.find('pay')>=0 or haystack.find('paid')>=0):
            haystack='how much payment part time work'
      elif haystack.find('student visa')>=0 and (haystack.find('how long')>=0 or haystack.find('appl')>=0):
            haystack='student visa application'
      elif haystack.find('change')>=0 and (haystack.find('tourist')>=0 and haystack.find('visa')>=0):
            haystack='change tourist visa'
      elif haystack.find('change')>=0 and haystack.find('working holiday')>=0 and haystack.find('visa')>=0:
            haystack='change working holiday visa'
      elif (haystack.find('work')>=0 or haystack.find('job')>=0) and haystack.find('after')>=0 and (haystack.find('graduat')>=0 or haystack.find('study')>=0):
            haystack='job after finish study'
      elif haystack.find('time')>=0 or haystack.find('jet lag')>=0 or haystack.find('difference')>=0 or haystack.find('wonder')>=0:
